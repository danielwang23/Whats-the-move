...
function Question2Page({ onNext, updateResponse }) {
...
//   };
    const toggleOption = (category) => {
        const newSelection = selectedOptions.includes(category)
        ? selectedOptions.filter((option) => option !== category)
        : [...selectedOptions, category];

        setSelectedOptions(newSelection);
        updateResponse('Activity', newSelection); // Update the centralized state with selected categories
    };
